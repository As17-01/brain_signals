...
def main(cfg: DictConfig) -> None:
    ...
    for i in range(cfg.num_folds):
        ...
        train_features = torch.from_numpy(features[train_index]).float().to(device, non_blocking=True)
        train_target = torch.from_numpy(target[train_index]).float().to(device, non_blocking=True)

        test_features = torch.from_numpy(features[test_index]).float().to(device, non_blocking=True)
        test_target = torch.from_numpy(target[test_index]).float().to(device, non_blocking=True)
